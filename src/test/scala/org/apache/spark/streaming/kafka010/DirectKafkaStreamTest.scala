...
class DirectKafkaStreamTest
  ...
  test("offset recovery") {
    ...
    stateStream.foreachRDD { (rdd: RDD[(String, Int)]) =>
      rdd.collect().headOption.foreach { x =>
        DirectKafkaStreamTest.total.set(x._2)
      }
    }
    ...
    eventually(timeout(10 seconds), interval(50 milliseconds)) {
      assert(DirectKafkaStreamTest.total.get === (1 to 10).sum)
    }
    ...
    eventually(timeout(10 seconds), interval(50 milliseconds)) {
      assert(DirectKafkaStreamTest.total.get === (1 to 20).sum)
    }
    ssc.stop()
  }
  ...
  test("Direct Kafka stream report input information") {
    ...
    import DirectKafkaStreamTest._
    ssc = new StreamingContext(sparkConf, Milliseconds(200))
